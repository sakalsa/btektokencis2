...
const TOKEN_METADATA_BASE_URL: &str = "https://bbt.tc/token/";
...
struct ViewState {
    ...
    tokens: Vec<(ContractTokenId, ViewTokenSupply)>,
}
...
#[receive(contract = "fungible-cis2", name = "view", return_value = "ViewState")]
fn contract_view<S: HasStateApi>(
    ...
    for v in state.tokens.iter() {
        let token_id = v.0.to_owned();
        tokens.push((
            token_id,
            ViewTokenSupply {
                circulating_supply: state.get_circulating_supply(&token_id)?,
                max_supply: state.get_token_supply(&token_id)?,
            },
        ));
    }
...
mod tests {
    ...
    fn test_mint() {
        ...
        // tokens: collections::BTreeMap<ContractTokenId, (TokenMetadata, TokenParams)>,
        // tokens: BTreeMap<TokenIdU8, (TokenMetadata, TokenParams), Global
        // and parameter.
        ...
        tokens.insert(
            ...
            (
                ...
                {
                    // let amount = 0.into();
                    // let max_supply = 0.into();
                    // TokenParams { amount, max_supply }
                    TokenParams {
                        amount: 0.into(),
                        max_supply: 0.into(),
                    }
